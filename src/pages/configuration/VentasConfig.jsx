...
export default function VentasConfig() {
  ...
  const handleAddAssignment = async (businessId, credentialId) => {
    ...
    try {
      // 1. Obtener la info de la credencial primero para tener el api_name
      ...
      setError("Error al obtener info de credencial: " + err.message);
      setLoading(false);
      return;
    }

    if (!apiData) {
      setError("No se encontró la credencial seleccionada.");
      setLoading(false);
      return;
    }

    try {
      // 2. Insertar la asignación incluyendo el api_name para filtrado rápido
      ...
      setError("Error al asignar credencial: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemoveAssignment = async (assignId) => {
    if (!window.confirm("¿Desvincular esta credencial de este negocio?"))
      return;
    setLoading(true);
    try {
  ...
  return (
    <Box>
        ...
        <List sx={{ mb: 4 }}>
          {devices.map((device) => (
            ...
            >
              <ListItemText
                primary={device.name}
                secondary={`ID: ${device.mp_device_id} — En: ${device.businesses?.name}`}
              />
              <ListItemSecondaryAction>
                <IconButton
      ...
      <Typography variant="h6" gutterBottom>
        Sucursales e Integraciones API
      </Typography>
      <Typography variant="body2" color="textSecondary" sx={{ mb: 3 }}>
        Conecta tus sucursales con las cuentas de Mercado Pago, Tus Facturas App, etc.
      </Typography>
            ...
            <Paper variant="outlined" sx={{ p: 2, height: "100%" }}>
              ...
              <Box sx={{ mt: 2, mb: 2 }}>
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mt: 1 }}>
                  {assignments.filter((a) => a.business_id === business.id)
                    .length === 0 ? (
              ...
              <TextField
                ...
                value=""
                onChange={(e) =>
                  handleAddAssignment(business.id, e.target.value)
                }
                disabled={loading || credentials.length === 0}
              >
                <MenuItem value="" disabled>
                  <em>Selecciona una credencial...</em>
                </MenuItem>
                {credentials
                  .filter(
                    (c) =>
