...
export default function FacturacionConfig() {
  ...
  const { fetchProvinces, fetchInvoiceTypes } = useInvoices();
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (bData?.length > 0) {
        ...
        // Cargar referencias desde TFA para el primer negocio
          ...
          <Card variant="outlined">
            <CardContent>
                  ...
                  <FormControl fullWidth>
                    ...
                    <Select value={formData.default_comprobante_tipo} label="Comprobante por Defecto" onChange={e => setFormData({...formData, default_comprobante_tipo: e.target.value})}>
                      {invoiceTypes.length > 0 ? 
                        invoiceTypes.map(t => <MenuItem key={t.id} value={t.id}>{t.nombre}</MenuItem>) :
                        [<MenuItem key="11" value="11">Factura C</MenuItem>]
                      }
                    </Select>
                  </FormControl>
                </Grid>
              ...
              <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end' }}>
                <Button variant="contained" startIcon={<SaveIcon />} onClick={handleSave} disabled={isSaving}>Guardar</Button>
              </Box>
            </CardContent>
          </Card>
        </Grid>
