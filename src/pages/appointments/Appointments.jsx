...
export default function Appointments() {
  ...
  const CAL_COM_LINK = "perpel-test/servicio-peluqueria"; 
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} lg={8}>
          <Paper sx={{ p: 0, overflow: 'hidden', height: 650, borderRadius: 2 }}>
             <Box 
               component="div"
               id="my-cal-inline"
          ...
          <Alert severity="info" sx={{ mt: 2 }}>
            Los turnos agendados aquí se sincronizarán automáticamente con el sistema mediante Webhooks.
          </Alert>
        </Grid>

        {/* Lado Derecho: Turnos Recientes */}
        <Grid item xs={12} lg={4}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>Próximos Turnos</Typography>
          <Paper sx={{ p: 2, height: 'auto', maxHeight: 650, overflowY: 'auto' }}>
            {loading ? (
                ...
                <Card key={appt.id} variant="outlined" sx={{ mb: 2 }}>
                  <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                       ...
                       <Alert severity="info" icon={false} sx={{ py: 0, px: 1, fontSize: '0.7rem' }}>
                         {appt.status}
                       </Alert>
                    </Box>
                    ...
                    <Typography variant="caption" sx={{ display: 'block' }}>
                      Cliente: <strong>{appt.user_profiles?.full_name || 'Desconocido'}</strong>
                    </Typography>
                  </CardContent>
                </Card>
              ))
