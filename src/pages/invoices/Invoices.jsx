...
export default function Invoices() {
  ...
  const [invoiceOptions, setInvoiceOptions] = useState({
    comprobante_tipo: "11",
    iva_id: "3", // ID 3 = 0% por defecto para Factura C
    punto_venta: 1,
    customer_name: "",
    customer_doc_number: "",
  ...
  useEffect(() => {
    ...
    const isB = typeId === "6" || typeLabel.toUpperCase().includes("FACTURA B");
    const isC =
      typeId === "11" || typeLabel.toUpperCase().includes("FACTURA C");
    const isA = typeId === "1" || typeLabel.toUpperCase().includes("FACTURA A");
  ...
  }, [invoiceOptions.comprobante_tipo, invoiceTypes]);
  ...
  const handleSendPointIntent = async () => {
    if (!invoiceToPay?.order_id || !selectedPointDeviceId) {
      setSnackbar({
        open: true,
        message: "Faltan datos para procesar el cobro",
        severity: "error",
      });
      return;
    }
  ...
  const handleSaveCustomer = async () => {
    ...
    if (result.success) {
      ...
      setInvoiceOptions((prev) => ({
        ...
        customer_doc_type:
          result.data.doc_type === "80"
            ? "CUIT"
            : result.data.doc_type === "96"
              ? "DNI"
              : "OTRO",
        iva_condition_id:
          result.data.iva_condition === "Responsable Inscripto" ? "RI" : "CF",
      }));
  ...
  const handleSelectOrder = async (order) => {
    ...
    setIsProcessing(true);
    const businessData = Array.isArray(order.businesses)
      ? order.businesses[0]
      : order.businesses;
    ...
    try {
      const [rates, conds, payments, docs, types, customersCF] =
        await Promise.all([
          vatRates.length === 0 ? fetchVatRates() : Promise.resolve(vatRates),
          ivaConditions.length === 0
            ? fetchIvaConditions(order.business_id)
            : Promise.resolve(ivaConditions),
          paymentConditions.length === 0
            ? fetchPaymentConditions(order.business_id)
            : Promise.resolve(paymentConditions),
          documentTypes.length === 0
            ? fetchDocumentTypes(order.business_id)
            : Promise.resolve(documentTypes),
          invoiceTypes.length === 0
            ? fetchInvoiceTypes(order.business_id)
      ...
      const initialType =
        initialCustomer?.doc_type === "80" ? "1" : String(defaultType);
      const initialIva =
        initialType === "6" || initialType === "11" ? "3" : "5";
      ...
      setInvoiceOptions({
        ...
        customer_name:
          initialCustomer?.full_name ||
          order.customer_name ||
          "Consumidor Final",
        customer_doc_number:
          initialCustomer?.doc_number || order.customer_doc_number || "0",
        customer_doc_type:
          (initialCustomer?.doc_type || order.customer_doc_type) === "80"
            ? "CUIT"
            : (initialCustomer?.doc_type || order.customer_doc_type) === "96"
              ? "DNI"
              : "OTRO",
        iva_condition_id:
          (initialCustomer?.iva_condition || order.iva_condition) ===
    ...
    if (result.success) {
      ...
      if (isArcaError) {
        setSnackbar({
          open: true,
          message:
            "Los servicios de ARCA (AFIP) están experimentando demoras. Por favor, intenta nuevamente en 10 minutos.",
          severity: "warning",
        });
      } else {
        setSnackbar({
  ...
  const handleShare = async (storagePath, method) => {
    ...
    if (method === "whatsapp")
      ...
      window.open(
        `mailto:?subject=Factura PerPel&body=${encodeURIComponent(text)}`,
        "_blank",
      );
  };
  ...
  const handleVoid = async (id) => {
    ...
    if (window.confirm("¿Anular fiscalmente?")) {
      ...
      if (result.success)
        setSnackbar({ open: true, message: "Anulada.", severity: "success" });
      setIsProcessing(false);
    }
  };
  ...
  const filteredInvoices = invoices.filter((inv) => {
    const clientName =
      inv.order?.customer_name || inv.order?.customers?.full_name || "";
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      valueGetter: (p, r) => `${r.punto_venta}-${r.cbte_nro}`,
    },
    {
      field: "business",
      headerName: "Sucursal",
    ...
    },
    {
      ...
      minWidth: 200,
      valueGetter: (p, r) =>
         r.order?.customers?.full_name ||  r.order?.customer_name || "N/A",
    },
    {
      field: "total_amount",
      headerName: "Total",
    ...
    {
      field: "fch_serv_vto_pago",
      headerName: "Vencimiento",
      width: 120,
      valueFormatter: (v) => (v ? new Date(v).toLocaleDateString() : "N/A"),
    },
    {
      field: "actions",
      headerName: "Acciones",
  ...
  return (
    <Box sx={{ width: "100%", p: 1 }}>
      ...
      >
              ...
              <Autocomplete
                ...
                onChange={(e, v) => {
                  ...
                  if (v)
                    setInvoiceOptions((prev) => ({
                      ...prev,
                      customer_name: v.full_name,
                      customer_doc_number: v.doc_number,
                      customer_doc_type:
                        v.doc_type === "80"
                          ? "CUIT"
                          : v.doc_type === "96"
                            ...
                            : "OTRO",
                      iva_condition_id:
                        v.iva_condition === "Responsable Inscripto"
                          ? "RI"
                          : "CF",
                    }));
                }}
                ...
                <Grid item xs={4}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setInvoiceOptions({
                        ...invoiceOptions,
                        customer_doc_type: e.target.value,
                      })
                    }
                  >
                    {documentTypes.map((d) => (
                ...
                <Grid item xs={8}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setInvoiceOptions({
                        ...invoiceOptions,
                        customer_doc_number: e.target.value,
                      })
                    }
                    disabled={invoiceOptions.customer_doc_type === "OTRO"}
                  />
                </Grid>
              </Grid>
              <TextField
                ...
                onChange={(e) =>
                  setInvoiceOptions({
                    ...invoiceOptions,
                    iva_condition_id: e.target.value,
                  })
                }
              >
                {ivaConditions.map((c) => (
            ...
            <Grid item xs={12} md={6}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  setInvoiceOptions({
                    ...invoiceOptions,
                    iva_id: e.target.value,
                  })
                }
                disabled={
                  String(invoiceOptions.comprobante_tipo) !== "1" &&
                  !String(invoiceOptions.comprobante_tipo).includes("FACTURA A")
                }
              >
                {vatRates.map((r) => (
              ...
              <TextField
                ...
                onChange={(e) =>
                  setInvoiceOptions({
                    ...invoiceOptions,
                    condicion_pago_id: e.target.value,
                  })
                }
              >
              ...
              <TextField
                ...
                margin="dense"
                value={
                  invoiceOptions.punto_venta_display ||
                  invoiceOptions.punto_venta
                }
                disabled={true}
              />
            </Grid>
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell align="right">
                        <b>$ {selectedOrder?.total_amount.toFixed(2)}</b>
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
        ...
        <DialogActions>
          ...
          <Button
            ...
            startIcon={
              isProcessing ? <CircularProgress size={20} /> : <ReceiptIcon />
            }
          >
            Emitir Factura
          </Button>
        ...
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <Box>
            <IconButton
              ...
              color={detailTab === 0 ? "primary" : "default"}
            >
              <VisibilityIcon />
            </IconButton>
            <IconButton
              onClick={() => setDetailTab(1)}
              color={detailTab === 1 ? "primary" : "default"}
            >
              <ListAltIcon />
            </IconButton>
            <IconButton
              onClick={() => setDetailTab(2)}
              color={detailTab === 2 ? "primary" : "default"}
            >
              <HistoryIcon />
            </IconButton>
          </Box>
            ...
            <>
              {detailTab === 0 && (
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <Typography variant="caption">Número Legal</Typography>
                    <Typography>
                      {selectedInvoice.punto_venta}-{selectedInvoice.cbte_nro}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={12}>
                    ...
                    <Typography>
                      {selectedInvoice.order?.customer_name || "N/A"}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
              ...
              {detailTab === 1 && (
                <Box>
                  ...
                  <DataGrid
                    ...
                    density="compact"
                    rows={
                      selectedInvoice.order?.order_items?.map((item, idx) => ({
                        ...item,
                        id: idx,
                      })) || []
                    }
                    columns={[
                      {
                        ...
                        headerName: "Producto",
                        flex: 1,
                        valueGetter: (p, r) => r.inventory_items?.name,
                      },
              ...
              {detailTab === 2 && (
                <Box>
                  <Typography variant="subtitle2">Historial API</Typography>
                  <DataGrid
                    autoHeight
                    density="compact"
                    rows={invoiceLogs.map((l) => ({ ...l, id: l.id }))}
                    columns={[
                      {
                        field: "created_at",
                        headerName: "Fecha",
        ...
        <DialogActions sx={{ justifyContent: "space-between" }}>
          <Box>
            <IconButton
              color="success"
              onClick={() =>
                handleShare(selectedInvoice.full_pdf_url, "whatsapp")
              }
            >
              <WhatsAppIcon />
            </IconButton>
            <IconButton
              color="primary"
              onClick={() => handleShare(selectedInvoice.full_pdf_url, "email")}
            >
              <EmailIcon />
            </IconButton>
          </Box>
          <Box>
            ...
            {selectedInvoice?.full_pdf_url && (
              <Button
                variant="contained"
                onClick={() => handleDownload(selectedInvoice.full_pdf_url)}
              >
                Descargar PDF
              </Button>
            )}
              ...
              },
              {
                ...
                renderCell: (p) => (
                  <Chip
                    label={p.value}
                    size="small"
                    color={p.value === "PAID" ? "success" : "error"}
                    variant="outlined"
                  />
                ),
              },
      ...
      >
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) =>
                  setCustomerForm({ ...customerForm, doc_type: e.target.value })
                }
              >
                <MenuItem value="80">CUIT</MenuItem>
                <MenuItem value="96">DNI</MenuItem>
                <MenuItem value="99">Sin Documento</MenuItem>
              </TextField>
            </Grid>
            <Grid item xs={6}>
              <TextField
                ...
                disabled={customerForm.doc_type === "99"}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                size="small"
                margin="dense"
                value={customerForm.iva_condition}
                onChange={(e) =>
                  setCustomerForm({
                    ...customerForm,
                    iva_condition: e.target.value,
                  })
                }
              >
        ...
        <DialogActions>
          <Button onClick={() => setOpenCustomerDialog(false)}>Cancelar</Button>
          <Button
            variant="contained"
          ...
          >
            ...
            >
              ...
              {invoiceToPay?.total_amount
                ? parseFloat(invoiceToPay.total_amount).toFixed(2)
                : "0.00"}
            </Typography>
          </Box>
              ...
              <Box>
                ...
                {intentStatus === "WAITING" ? (
                  ...
                  <Box sx={{ textAlign: "center", p: 3 }}>
                    ...
                    <Button variant="contained" onClick={handleClosePayment}>
                      Aceptar
                    </Button>
                  </Box>
                ) : (
