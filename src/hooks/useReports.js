...
export const useReports = () => {
  ...
  const cancelOrder = async (orderId, origin) => {
    setLoading(true);
    setError(null);
    try {
      const { data: rpcResult, error: rpcError } = await supabase.rpc('cancel_order', { p_order_id: orderId });
      if (rpcError) throw rpcError;
      if (rpcResult.status === 'error') throw new Error(rpcResult.message);

      if (origin === 'TIENDANUBE') {
        try {
          const { data: tnResult } = await supabase.functions.invoke('tn-cancel-order', {
            body: { orderId }
          });
          if (tnResult?.success === false) {
            console.warn("La orden se anuló en el ERP pero falló la notificación a Tiendanube:", tnResult.message);
          }
        } catch (tnErr) {
          console.error("Error al invocar tn-cancel-order:", tnErr);
        }
      }
      ...
      setError(err.message || "Error al anular la orden.");
